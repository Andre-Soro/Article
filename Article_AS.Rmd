...
Wood cores were extracted to a thickness of 1,68 mm from the south facing side of each tree 15-year-old trees in 2015 with a breast-height diameter (DBH15, 1.3 m above ground). Tree cores are extracted using increment borer which is a specialized tool for extracting wood cores from a living tree with relatively minor damage to the plant itself. These cores were used to determine ring width and wood density. Density was determined by scanning incremental cores from pith-to-bark along the radial face at a resolution of 25 $\mu$m in the QTRS-01X ring analyzer (Quintek Measurement Systems Inc., Knoxville, TN, USA). A weighted mean density line was obtained by summing the product between the annual density measurements and the ratio of the respective radial areas to the total radial area. Wood cores were conditioned to about 7% moisture before scanning. Thus, the determination of the wood density makes it possible to calculate a quality index of it. From the density profiles, the limits of the annual growth rings were determined together with the relative proportions of earlywood and latewood in each annual ring using in-house data processing routines. Ring width determined through information of densitometry profiles. The transition between earlywood and the latewood is determined using the "Unix" script. This script used a transition density threshold ($D_t$) determined for each ring as follows:

$$D_t= \frac{(D_{min}+D_{max})}{2}$$
Where $D_t$ is ring density threshold, $D_{min}$ is ring minimum density and $D_{max}$ is ring maximum density. Latewood started when these conditions were met:
...
Where $D_x$ is current reading and $D_t$ is next reading. \ 
...
After subtraction of the typical density, it is observed that the residues remain variable. Knowing that the low frequency signal is subtracted, this radial variation can only be due to the high frequency signal which is the interannual radial variation. As explained earlier in the introduction, low interannual radial variation is an important criterion for wood quality. In order to identify tree families with low interannual radial variation amplitude in density, we have created a interannual radial variation amplitude index.
The interannual variation amplitude of wood density index is an index to determine the variability in the density of pith-to-bark in order to identify tree families that are least sensitive to variations in climatic conditions. The residues of our model were used to obtain the interannual variation amplitude of wood density index, which was determined using an equation. We subtracted the density residues values of year $n-1$ from year $n$. We were interested more specifically in the difference between the average density residues of annual rings of two consecutive years, no matter whether the difference was positive or negative. The absolute value was used for each difference to get rid of the sign. The index was estimated from the following formula:

$$V = \sum | D_n - D_{n - 1} |$$

Where V is the interannual radial variation amplitude of wood density index, $D_n$ is the wood density residues of year $n$ and  $D_{n - 1}$ is the wood density residues of year $n-1$. 

## Spearman’s rank correlation coefficient (Spearman’s rho) 
Spearman’s rank correlation coefficient is a correlation coefficient to describe the link between two variables by means of a monotonic function. We therefore used a Spearman’s rank correlation coefficient to make a comparison of the response of the tree families between the two study sites. We calculated this coefficient for the interannual variation amplitude of wood density index and the different parameters of the model. The elements for which we calculated Spearman’s rho are: the interannual variation amplitude of wood density index for each of the tree families between two study sites and the three parameters to estimate from the model. The $cor.test$ function in the R [@R2018] statistical programming environment was used to calculate Spearman’s rho. We used a 95 percent confidence interval.
...
Variance and covariance for genetics were estimated using linear mixed-effects model [@Lenz2013] of the R software. The equation used is as follows:
...
The model we have developed has made it possible to optimize the density of the wood for each core. A summary of the parameter values obtained of the model fitting process (Eq. 1) can be found in Table 2. We observe that the average value of the model parameters differs between Asselin and Saint-Casimir. For the parameters $b_0$ and $b_1$, the average is higher in Saint-Casimir and for parameter $b_2$ it is higher in Asselin. Although there are small differences in the parameters between the two sites, these differences are very small. The fit of the model is therefore similar at both sites.\ 
...
When considering all the cores of trees from the pith-to-bark, ring density was high near the pith and decreased rapidly in the first 5 growth rings. It declined from 430 kg/m^3^ to 370 kg/m^3^ for the Saint-Casimir site and from 455kg/m^3^ to 375 kg/m^3^ for the Asselin site. Then, this decrease was followed by a slow increase until the twelfth year. \ 

Model predictions are similar to data trends across sites (figs 4 and 5). A decrease in density is observed in the first years of growth and then an increase in density from a certain cambial age. Whether it is the data or the predictions of the model, we observe a difference between the Asselin (fig 4) and Saint-Casimir (fig 5) sites. The increase in density after the decrease of the first years of growth is greater in Saint-Casimir than in Asselin. However, the average density is very similar between the two sites. In spite of the interannual variations, the trees have the same general tendency and this one seems well described by the model which adapts to each of the sites.\ 
...
|Site                     |Min  |Mean  |Max    |
|:------------------------|----:|-----:|------:|
|Asselin (Témiscouata)    |116.5| 417.9| 1155.8|
|Saint-Casimir (Portneuf) | 54.9| 339.8|  746.0|
|Combined                 | 54.9| 375.9| 1155.8|
Table: Summary table of variability index

When comparing the index of variability between study sites, there is greater variability in Asselin than in Saint-Casimir. A summary of the density interannual amplitude index values can be found in table 3. The maximum value is 1155.8 in Asselin, and 746 for the Saint-Casimir site, which represents a huge difference. \ 
...
The results of the rank correlation coefficient are summarized in Table 4. These results show an absence of correlation for the variability index (p-value = 0.43) and a strong correlation for all the parameters of the model (p-value = 2.2 10^-16^). The value of Rho is very similar between the three parameters. \ 
