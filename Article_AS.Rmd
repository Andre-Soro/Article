...
![legend of this figure](graph_table/parameters.png)
...
Figure 1 : Graphic of description of different parameters $b_0$, $b_1$ and $b_2$ of the model. Increasing the value of the parameters $b_0$ and $b_1$ results in an increase in the density at the level of all the rings. The increase in the parameter $b_2$ causes a sharp rise in density after the decrease in the first rings.


It can be observed in figure 1 that the parameters $b_0$ and $b_1$ influence the whole of the curve. An increase of one of these parameters leads to that of the density on all rings. Unlike parameter $b_0$ and $b_1$, parameter $b_2$ seems to be influenced by some of the density. A low value of this parameter causes a sharp increase of the density starting from the third ring and when this parameter increases a decrease of the density is observed at this level until the curve is similar to the cases of the parameters $b_0$ and $b_1$. Beyond the value 1, the parameter $b_2$ does not influence the variation of the density.\ 
...
Where H is the uniformity index, $D_n$ is the wood density residues of year $N$ and  $D_n - 1$ is the tree density residues of year $N-1$. 
...
where yijkl  is the observation on the ijklth tree, $\mu$ is the overall mean, si is the fixed site effect, $b_{(i)j}$ is the fixed effect of the $j$ th bloc in site $i$, $f_k$ is the random effect of the k th family, $bf_{(i)j}$ is the random interaction of family and bloc, and $e_{ijkl}$ is the residual error.
