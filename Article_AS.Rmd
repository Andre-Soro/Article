...
However, when modeling density profiles we found that the variation between individual discs follows different patterns from the pith-to-bark between tree. We noticed that it was impossible to optimize the equation using the same estimated parameters for all tree cores, because the pattern of variation of annual rings average density of each tree is more or less different from the others. To work around this problem, we chose to model each of the profiles separately. We, therefore, used the optim function of the stats package in the R [@R2018] statistical programming environment to find the best parameters iteratively for each tree core . The fit of the model was evaluated by visual analysis of the model residual curves against adjusted values in addition to the explanatory variables, fit indices (R^2^) [@Pape1999].
...
## Determination of interannual variation amplitude of wood density index
After subtraction of the typical density, it is observed that the residues remain variable. Knowing that the low frequency signal is subtracted, this variation can only be due to the high frequency signal which is the interannual variation. As explained earlier in the introduction, low interannual variation is an important criterion for wood quality. In order to identify tree families with low interannual variation amplitude in density, we have created a viability index.
...
Where V is the interannual variation amplitude of wood density index, $D_n$ is the wood density residues of year $n$ and  $D_{n - 1}$ is the wood density residues of year $n-1$. 
