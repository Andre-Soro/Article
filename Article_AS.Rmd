...
Variance components for genetic analysis were estimated using ASReml 3 [@Butler2018] based on linear mixed-effects model using as follows:
...
The model we have developed has made it possible to adjust wood density for each core. A summary of the parameter values obtained of the model fitting process (Eq. 1) can be found in figure 4&5. We observe that the average value of the model parameters differs between Asselin and Saint-Casimir. For the parameters $b_0$ and $b_1$, the average is higher in Saint-Casimir and for parameter $b_2$ it is higher in Asselin. Although there are small differences in the parameters between the two sites, these differences are very small. The fit of the model is therefore similar at both sites. 

![Summary graphic of parameters in Asselin.](graph_table/parameter.asselin.png){width=335px; height=330px} 


    

![Summary graphic of parameters in Saint-Casimir.](graph_table/parameters.casimir.png){width=335px; height=330px} 
...
Model predictions are similar to data trends across sites (figs 6 and 7). A decrease in density is observed in the first years of growth and then an increase in density from a certain cambial age. Whether it is the data or the predictions of the model, we observe a difference between the Asselin (fig 6) and Saint-Casimir (fig 7) sites. The increase in density after the decrease of the first years of growth is greater in Saint-Casimir than in Asselin. However, the average density is very similar between the two sites. In spite of the interannual variations, the trees have the same general tendency and this one seems well described by the model which adapts to each of the sites.\ 
...
![Summary graphic of index V per site.](graph_table/indexV.png){width=335px; height=330px}


When comparing the index V between study sites, there is greater variability in Asselin than in Saint-Casimir. A summary of index V values can be found in figure 8. The maximum value is 1155.8 in Asselin, and 746 for the Saint-Casimir site, which represents a huge difference. \ 
...
The results of the rank correlation coefficient are summarized in Table 2. These results show an absence of correlation for the index V (p-value = 0.43) and a strong correlation for all the parameters of the model (p-value = 2.2 10^-16^). The value of Rho is very similar between the three parameters. \ 

![Abstract of the heritability ($hi^2$) of the density, the ring width, parameters $b_0$, $b_1$, $b_2$, index V and standard error per site.](graph_table/heritability.png){width=335px; height=330px}



Heritability is higher in each site separately than that of the combined sites for all traits (fig 9). Wood density is the only character with strong heritability for each of the sites as well as for the combined sites. The annual ring width and the parameter $b_0$ have low heritability. When parameters $b_1$ and $b_2$, their heritability is very low. The uniformity index has almost zero heritability regardless of the study site.\ 
